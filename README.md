...
| 11 | **日本語テキスト正規化・OMML LaTeX変換** | ✅ | スタイル名の全角→半角正規化（`見出し１`→`見出し1`）、OMML数式の LaTeX 変換（`\frac{}`・`^{}`・`_{}`・`\sqrt{}`）。テーブルセル内 `<br>` 変換も対応 |
| 12 | **見出し検出の柔軟化** | ✅ | `heading_styles` キーに `"prefix:<str>"` / `"regex:<pattern>"` 記法で前方一致・正規表現マッチを指定可能 |
| 13 | **進捗表示（CLI）** | ✅ | `indicatif` によるアニメーションプログレスバー。経過時間・処理中ファイル名をリアルタイム表示。完了後に成功/失敗の一覧を出力 |
